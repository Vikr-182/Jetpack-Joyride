...
class person():
    def __init__(self,x_cordinate,y_cordinate):
        self._xco = x_cordinate
        self._yco = y_cordinate
        self._life = 10
        self._coins = 0
        self._vel = 0 # Velocity along y direction ( either 1+ or (var)-)
        self._shield = 0
        self._bullets = []
        self._dragon = 0
        self._f = ["" for i in range(4)]
        for i in range(4):
            self._f[i] = extract("bonus"+str(i)+".txt")
        self._count = 0
...
class Mandalorian(person):
    def __init__(self,xco,yco):
        ...
        person.__figure = [[["~",Fore.YELLOW+Back.GREEN+"O"+Style.RESET_ALL,"~"],["~",Fore.YELLOW+Back.GREEN+"O"+Style.RESET_ALL,"~"]],
                         [[" ","|"," "],[" ","|"," "]],
                         [["/","|"," "],[" ","|","\\"]],
                        [["|"," ","\\"],["/"," ","|"]]]
        person.__alive = True
    ...
    #    # Act gravity 
        ...
        for i in range(final):
            for j in range(3):
                if game_board[self.get_yco()+4+i][self.get_xco()+j] in not_allowed_collision:
                    self.set_life(self.get_life()-1)
                    correct_beams(game_board,self.get_yco()+4+i,self.get_xco()+j,3)
                elif game_board[self.get_yco()+i+4][self.get_xco()+j] in coins:
                    # print("SSSSSSSSSSSSSSSSSSSSSSSSSSSSS")
                    self.set_coins(self.get_coins()+1)
                elif game_board[self.get_yco()+4+i][self.get_xco()+j] in powerup:
                    speed[0] = 1
    ...
    def move_bullets(self,game_board,din):
        for i in range(len(self.get_bullets())):
            ...
            if len(self.get_bullets()) > i  and self.get_bullets()[i][0] <= 2:
                ...
                game_board[self.get_bullets()[i][1]][self.get_bullets()[i][0]] = " "
                game_board[self.get_bullets()[i][1]][self.get_bullets()[i][0]-2] = Fore.BLUE+"o"+Style.RESET_ALL
...
def interact_magnet(game_board,xpos,xdim,din):
    ...
    while anaa is 0:
        for i in range(20):
            for j in range(xpos,xpos+xdim):
                if game_board[i][j]==Fore.RED+Back.WHITE+"U"+Style.RESET_ALL:
                    anaa = -1
                    magy = i
                    magx = j
        if anaa is 0:
    ...
    if magx < din.get_xco() and anaa is -1:
        ...
        for i in range(4):
            for k in range(2):
                # print(game_board[din.get_yco()+i][din.get_xco()-1-k],end="")
        ...
        for i in range(4):
            for k in range(2):
                game_board[din.get_yco()+i][din.get_xco()+1+k] = " "
        ...
        for i in range(4):
            for k in range(2):
                # print("ANNA"+game_board[din.get_yco()+i][din.get_xco()+3+k],end="|")
